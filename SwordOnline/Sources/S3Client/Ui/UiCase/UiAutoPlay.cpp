	...
	case WND_N_BUTTON_CLICK:
		...
		else if (uParam == (unsigned int)(KWndWindow*)&m_MoveMpsCK)
		{
			if (m_MoveMpsCK.IsButtonChecked())
			{
				...
				g_pCoreShell->PAIOperation(GPI_FOLLOW_PEOPLE_RADIUS_IS_CHECK, TRUE, NULL, NULL); //pius Check move map
			...
			else
			{
				m_MoveMpsCK.CheckButton(false);
				g_pCoreShell->PAIOperation(GPI_AUTO_MOVEMPS, FALSE, NULL, NULL);
				g_pCoreShell->PAIOperation(GPI_FOLLOW_PEOPLE_RADIUS_IS_CHECK, FALSE, NULL, NULL); //pius Check move map
            }
		}
