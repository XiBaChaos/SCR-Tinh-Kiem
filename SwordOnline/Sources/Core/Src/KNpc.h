...
// Npc的装备（决定客户端的换装备）	
	...
	BOOL				m_bActivateAutoMoveBarrier1; //pius Them check auto move barrier
	BOOL				m_bActivateAutoMoveBarrier2; //pius Them check auto move barrier
	BOOL				m_bActivateAutoMoveBarrier3; //pius Them check auto move barrier
	BOOL				m_bActivateAutoMoveBarrier4; //pius Them check auto move barrier

	//pius Them check auto
	...
	KSyncPos			m_sSyncPos; //pius lag pos Npc
#endif
private:
...
private:
	...
	void				DoRun();
	//void				OnRun();
	void				OnRun(int nAddSpeed = 0); //pius fix auto xy map int nAddSpeed=0
	...
	void				OnHurt();
	//TamLTM
	void				ActiveAutoMoveBarrier(int moveX, int moveY); //pius Fix move toa do xy player.
...
//	有关格斗技能的------------------------------------------
	...
	BOOL				DoGoAttack(); //pius fix auto xy map
	void				OnGoAttack(); //pius fix auto xy map
...
public:
	...
	//pius Fix move toa do xy player.
