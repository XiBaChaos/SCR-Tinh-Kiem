...
void KNpc::Init()
{
	...
	m_nFigure = -1;
	m_nTeamServerID = -1;
	m_nCheckAutoMoveBarrier = 0; // pius check auto move barrier
	m_nMoveToFlagMiniMapX = 0; // pius check auto move barrier
	m_nMoveToFlagMiniMapY = 0; // pius check auto move barrier
	memset(&m_sSyncPos, 0, sizeof(m_sSyncPos)); //pius giai phong
#endif
...
void KNpc::OnRun(int nAddSpeed) //pius add int nAddSpeed fix auto xy map
{	
