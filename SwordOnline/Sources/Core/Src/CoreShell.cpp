...
public:
	...
	//pius start
	void XepHinhInvite(void* pPlayer);
...
//pius start
void KCoreShell::XepHinhInvite(void* pPlayer)
{
	//todo
	KUiPlayerItem* p = (KUiPlayerItem*)pPlayer;
	if (p)
	{
		if (p->nIndex >= 0 && p->nIndex < MAX_NPC && !Player[CLIENT_PLAYER_INDEX].CheckTrading())
		{
			Player[CLIENT_PLAYER_INDEX].XepHinhInvite(p->nIndex);
		}
	}
}
